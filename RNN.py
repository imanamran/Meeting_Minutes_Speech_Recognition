...
def decode_audio(audio_binary):
  ...
  return tf.squeeze(audio, axis=-1) #drop channels axis due to it being mono channel
...
files_ds = tf.data.Dataset.from_tensor_slices(train_files) # creates dataset of tensors
...
def get_spectrogram(waveform):
  ...
  zero_padding = tf.zeros(
      [16000] - tf.shape(waveform), # pads waveforms if less than 16000 bits
      dtype=tf.float32) 
...
# changes input shape to match RNN for each dataset (train, test, validate)
for spectrogram, _ in spectrogram_ds:
  spectrogram = spectrogram.numpy().reshape(1, 124, 129)

for spectrogram, _ in val_ds:
  spectrogram = spectrogram.numpy().reshape(spectrogram.shape[0], 124, 129)

for spectrogram, _ in test_ds:
  spectrogram = spectrogram.numpy().reshape(1, 124, 129)

# sets number of classes to classify each file to
...
model = models.Sequential([ # provides training
    layers.Input(shape=(124, 129)), # declares input shape
    norm_layer, # normalises
    layers.LSTM(128, input_shape=(124,129), return_sequences=True), # LSTM layer with 128 nodes. Return sequences makes sure the output shape is the same as input
    layers.LSTM(32, input_shape=(124,129)), # LSTM layer with 32 nodes
...
model.compile(
    optimizer=tf.keras.optimizers.Adam(), # optimiser
...
# save model if accuracy more than 85% (just for testing)
#if test_acc >= 0.85:
  #model.save('LSTM_Adam_big/')
